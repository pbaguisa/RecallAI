...
def call_llm(prompt, context=""):
    ...
    # Estimate tokens and cost (Gemini Flash is free but we track for learning)
...
def query():
    ...
    try:
        ...
        if mode == 'quiz':
            prompt = f"Generate ONE quiz question about: {user_query}\n\nThe question should test understanding of the concepts in the provided lecture content. Make it specific and answerable from the slides."
        else:
            prompt = user_query
        ...
        log_request(user_query, mode, "RAG", start_time, 0, 0, {"error": str(e)})
    ...
    <script>
        ...
        document.getElementById('fileInput').addEventListener('change', async (e) => {
            ...
            for (const file of files) {
                ...
                document.getElementById('uploadStatus').innerHTML = '<div class="loading">Uploading ' + file.name + '...</div>';
                ...
                try {
                    ...
                    if (data.success) {
                        ...
                        document.getElementById('uploadStatus').innerHTML = '<div class="error">' + data.error + '</div>';
                    }
                } catch (error) {
        ...
        function updateFilesList() {
            ...
            list.innerHTML = '<h3>Uploaded Files:</h3>' + 
                uploadedFiles.map(f => '<div class="file-item">✓ ' + f + '</div>').join('');
        }
        ...
        document.getElementById('submitBtn').addEventListener('click', async () => {
            ...
            if (!query) {
                document.getElementById('responseArea').innerHTML = '<div class="error">Please enter a question.</div>';
                return;
            }
            
            if (uploadedFiles.length === 0) {
                document.getElementById('responseArea').innerHTML = '<div class="error">Please upload PDFs first.</div>';
                return;
            }
            
            document.getElementById('submitBtn').disabled = true;
            document.getElementById('responseArea').innerHTML = '<div class="loading">🤔 Thinking...</div>';
            ...
            try {
                ...
                if (data.error) {
                    document.getElementById('responseArea').innerHTML = '<div class="error">' + data.error + '</div>';
                } else {
                    ...
                    html += '<p style="color: #999; font-size: 0.9em; margin-top: 10px;">⚡ Responded in ' + data.latency_ms + 'ms</p>';
                    
                    document.getElementById('responseArea').innerHTML = html;
                }
            } catch (error) {
                document.getElementById('responseArea').innerHTML = '<div class="error">Request failed: ' + error.message + '</div>';
            } finally {
        ...
        async function forfeit() {
            document.getElementById('responseArea').innerHTML += '<div class="loading">Getting answer...</div>';
            ...
            try {
                ...
                if (data.error) {
                    ...
                    document.getElementById('responseArea').innerHTML += '<div class="response-box"><strong>✅ Answer:</strong><br><br>' + data.answer.replace(/\n/g, '<br>') + '</div>';
                }
            } catch (error) {
                document.getElementById('responseArea').innerHTML += '<div class="error">Failed to get answer</div>';
            }
        }
